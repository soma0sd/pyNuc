...
def get_level_card(reset: bool=False):
  import os
  import pickle as pkl
  if not os.path.exists('LevelCards.pkl') or reset:
    import T01MiningLvCrad
    print('create level card data')
    T01MiningLvCrad.run()
    with open('LevelCards.pkl', 'rb') as f:
      return pkl.load(f)
  else:
    with open('LevelCards.pkl', 'rb') as f:
      return pkl.load(f)
